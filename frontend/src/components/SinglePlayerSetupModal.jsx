...
const DIFFICULTIES = [
    {
        ...
        label: 'Easy',
        icon: '🟢',
        desc: 'Short, common words. Great for warming up.',
        color: '#4ade80',
    },
    {
        id: 'medium',
        label: 'Medium',
        icon: '🟡',
        desc: 'Moderate length. A fair challenge.',
        color: '#facc15',
    },
    {
        id: 'hard',
        label: 'Hard',
        icon: '🔴',
        desc: 'Long, obscure words. Experts only.',
        color: '#f87171',
    },
    {
        id: 'random',
        ...
        <div className="spsm-overlay" onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
            <div className="spsm-panel">
                ...
                <div className="spsm-section">
                    ...
                    <div className="spsm-diff-grid">
                        {DIFFICULTIES.map(d => (
                            <button
                                key={d.id}
                                className={`spsm-diff-card ${difficulty === d.id ? 'active' : ''}`}
                ...
                <div className="spsm-section">
                    ...
                    <div className="spsm-rounds">
                        {ROUND_OPTIONS.map(n => (
                            <button
                                key={n}
                                className={`spsm-round-btn ${!isCustom && rounds === n ? 'active' : ''}`}
                                onClick={() => handleRoundOption(n)}
                            >
                                {n}
                            </button>
                        ))}
                        <button
                            className={`spsm-round-btn ${isCustom ? 'active' : ''}`}
                            onClick={() => handleRoundOption('custom')}
                        >
                            Custom
                        </button>
                    </div>
                    {isCustom && (
                        <input
                            className="spsm-input"
                            type="number"
                            placeholder="Rounds (1–50)"
                            min="1" max="50"
                            value={customRounds}
                ...
                <div className="spsm-footer">
                    ...
                    <button className="spsm-start-btn" onClick={handleStart} disabled={isDisabled}>
                        Deploy →
                    </button>
                </div>
            </div>
            ...
            <style>{`
                ...
                .spsm-close:hover { border-color: rgba(255,255,255,0.35); color: #fff; }
                .spsm-error {
                ...
                .spsm-label {
                    ...
                    font-size: 10px; letter-spacing: 0.2em; color: rgba(255,255,255,0.35);
                    text-transform: uppercase; font-family: 'Courier New', monospace;
                    margin: 0 0 16px;
                }
                .spsm-step { color: #00ffc8; font-weight: 700; }
                .spsm-diff-grid {
                ...
                .spsm-diff-card:hover {
                    background: rgba(255,255,255,0.05);
                    border-color: rgba(255,255,255,0.18);
                }
                .spsm-diff-card.active {
                ...
                .spsm-round-btn:hover { border-color: rgba(255,255,255,0.25); color: #fff; }
                .spsm-round-btn.active {
                ...
                .spsm-input:focus { border-color: rgba(0,255,200,0.4); }
                ...
                .spsm-start-btn:hover:not(:disabled) { background: #fff; transform: translateY(-1px); }
                ...
                @media (max-width: 480px) {
                    ...
                    .spsm-diff-grid { grid-template-columns: 1fr 1fr; }
